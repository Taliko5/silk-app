...
export function Stage({ id, index, canDelete, canReorder, isOpen: isOpenDefault = false, stagesCount }) {
  /**
   *
   * @param {number} index
   * @returns {string}
  ...
  const [{ handlerId, isDragging, handleKeyDown }, stageRef, dropRef, dragRef, dragPreviewRef] = useDragAndDrop(
    canReorder,
    {
      index,
      item: {
        ...
        >
          ...
          <AccordionContent padding={6} background="neutral0" hasRadius>
            <Grid gap={4}>
              ...
              <GridItem col={6}>
                <Field error={colorMeta?.error ?? false} name={colorField.name} id={colorField.name} required>
