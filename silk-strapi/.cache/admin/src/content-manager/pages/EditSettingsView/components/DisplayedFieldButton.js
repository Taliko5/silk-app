...
const DisplayedFieldButton = ({
  ...
  if (dropRef.current && clientOffset) {
    ...
    showLeftCarret =
      isOver && getItem.size !== 12 && Math.abs(clientOffset.x - hoverBoundingRect.left) < hoverBoundingRect.width / 2;
    showRightCarret =
      isOver && getItem.size !== 12 && Math.abs(clientOffset.x - hoverBoundingRect.left) > hoverBoundingRect.width / 2;
    ...
    <GridItem col={size}>
      ...
      >
        ...
        >
          ...
          {!isHidden && (
            <FieldButtonContent attribute={attribute} onEditField={onEditField} onDeleteField={onDeleteField}>
              {children}
            </FieldButtonContent>
          )}
        </CustomFlex>
