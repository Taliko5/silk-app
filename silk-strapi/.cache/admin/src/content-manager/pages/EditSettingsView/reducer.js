    ...
    switch (action.type) {
      ...
      case 'ON_ADD_FIELD': {
        ...
        if (Array.isArray(rowContentToSet)) {
          ...
          set(newList, [listSize > 0 ? listSize - 1 : 0, 'rowContent'], [{ name: action.name, size }]);
        }
      ...
      case 'REORDER_DIFF_ROW': {
        const actualRowContent = get(state, [...layoutPathEdit, action.dragRowIndex, 'rowContent'], []);
        const targetRowContent = get(state, [...layoutPathEdit, action.hoverRowIndex, 'rowContent'], []);
        const itemToInsert = get(state, [...layoutPathEdit, action.dragRowIndex, 'rowContent', action.dragIndex], {});
      ...
      case 'REORDER_ROW': {
        const newState = cloneDeep(state);
        const rowContent = get(newState, [...layoutPathEdit, action.dragRowIndex, 'rowContent'], []);
      ...
      case 'SUBMIT_META_FORM': {
        set(draftState, ['modifiedData', 'metadatas', state.metaToEdit, 'edit'], state.metaForm.metadata);
