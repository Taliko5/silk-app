...
import { useNotification, useTracking, ConfirmDialog, Link, useFetchClient } from '@strapi/helper-plugin';
import { useIntl } from 'react-intl';
import { Box, Divider, Layout, HeaderLayout, ContentLayout, Main, Button } from '@strapi/design-system';
...
const ListSettingsView = ({ layout, slug }) => {
  ...
  const [reducerState, dispatch] = useReducer(reducer, initialState, () => init(initialState, layout));
  ...
  const submitMutation = useMutation((body) => put(`/content-manager/content-types/${slug}/configuration`, body), {
    onSuccess() {
  ...
  const listRemainingFields = Object.entries(attributes)
    .filter(([name, attribute]) => checkIfAttributeIsDisplayable(attribute) && !displayedFields.includes(name))
    .map(([name]) => name)
    .sort();
      ...
      <Main aria-busy={isSubmittingForm}>
        <form onSubmit={handleSubmit}>
          <HeaderLayout
            ...
            primaryAction={
              <Button size="S" startIcon={<Check />} disabled={isEqual(modifiedData, initialData)} type="submit">
          ...
          <ContentLayout>
            <Box
              ...
              paddingRight={7}
            >
              <Settings modifiedData={modifiedData} onChange={handleChange} sortOptions={sortOptions} />
