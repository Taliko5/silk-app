...
import { CheckPagePermissions, LoadingIndicatorPage, AnErrorOccurred, useGuidedTour } from '@strapi/helper-plugin';
...
function renderDraglayerItem({ type, item }) {
  ...
  switch (actualType) {
    ...
    case ItemTypes.RELATION:
      return <RelationDragPreview displayedValue={item.displayedValue} status={item.status} width={item.width} />;
...
const App = () => {
  ...
  const authorisedModels = sortBy([...collectionTypeLinks, ...singleTypeLinks], (model) => model.title.toLowerCase());
  ...
  if (!contentTypeMatch && authorisedModels.length > 0) {
    return (
      <Redirect to={`${authorisedModels[0].to}${authorisedModels[0].search ? `?${authorisedModels[0].search}` : ''}`} />
    );
  }
    ...
    <Layout sideNav={<LeftMenu />}>
      ...
      <ModelsContext.Provider value={{ refetchData }}>
        <Switch>
          ...
          <Route path="/content-manager/collectionType/:slug" component={CollectionTypeRecursivePath} />
...
export default function () {
  ...
  return (
    <>
      <Helmet title={formatMessage({ id: getTrad('plugin.name'), defaultMessage: 'Content Manager' })} />
      <App />
    </>
  );
}
