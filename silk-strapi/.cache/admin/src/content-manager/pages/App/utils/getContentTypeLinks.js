...
const getContentTypeLinks = async ({ models, userPermissions, toggleNotification }) => {
  ...
  try {
    ...
    const singleTypeLinksPermissions = await Promise.all(checkPermissions(userPermissions, singleTypeSectionLinks));
    const authorizedSingleTypeLinks = singleTypeSectionLinks.filter((_, index) => singleTypeLinksPermissions[index]);
