        ...
        if (!value) {
          ...
          switch (attributes[name].type) {
            case 'component': {
              ...
              const mainFieldName = get(state, ['components', componentName, 'settings', 'mainField'], null);
              const mainFieldAttribute = get(state, ['components', componentName, 'attributes', mainFieldName]);
          ...
          draftState.displayedHeaders = state.displayedHeaders.filter((header) => header.name !== name);
        }
