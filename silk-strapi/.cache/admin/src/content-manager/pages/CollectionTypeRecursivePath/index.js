...
const CollectionTypeRecursivePath = ({
  ...
  const renderRoute = (
    ...
    return <Component slug={slug} layout={layout} state={state} goBack={goBack} id={id} origin={origin} />;
  };
  ...
  ].map(({ path, comp }) => <Route key={path} path={`${url}/${path}`} render={(props) => renderRoute(props, comp)} />);
        ...
        <Switch>
          <Route path={`${url}/configurations/list`}>
            <CheckPagePermissions permissions={cmPermissions.collectionTypesConfigurations}>
              <ListSettingsView layout={rawContentTypeLayout} slug={slug} updateLayout={updateLayout} />
