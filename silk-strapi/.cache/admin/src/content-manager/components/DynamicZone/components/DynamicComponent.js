...
import { Accordion, AccordionToggle, AccordionContent, IconButton, Box, Flex } from '@strapi/design-system';
...
const DynamicZoneComponent = ({
  ...
  const [{ handlerId, isDragging, handleKeyDown }, boxRef, dropRef, dragRef, dragPreviewRef] = useDragAndDrop(
    isFieldAllowed,
    {
      ...
      <StyledBox ref={composedBoxRefs} hasRadius>
        {isDragging ? (
          ...
          <Accordion expanded={isOpen} onToggle={handleToggle} size="S" error={errorMessage}>
            <AccordionToggle action={accordionActions} title={`${friendlyName}${mainValue}`} togglePosition="left" />
            <AccordionContent>
              <AccordionContentRadius background="neutral0">
                <FieldComponent componentUid={componentUid} icon={icon} name={`${name}.${index}`} isFromDynamicZone />
