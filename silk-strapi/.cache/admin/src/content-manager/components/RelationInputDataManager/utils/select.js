...
function useSelect({ componentUid, isUserAllowedToEditField, isUserAllowedToReadField, name, queryInfos }) {
  const {
    isCreatingEntry,
  ...
  const relationFetchEndpoint = useMemo(() => {
    ...
    if (componentUid) {
      ...
      return componentId ? getRequestUrl(`relations/${componentUid}/${componentId}/${fieldNameKeys.at(-1)}`) : null;
    }
