...
const EditViewDataManagerProvider = ({
  ...
  }, [initialValues, currentContentTypeLayout, components, setModifiedDataOnly, reduxDispatch, previousInitialValues]);
  ...
  const addNonRepeatableComponentToField = useCallback((keys, componentLayoutData, allComponents) => {
    dispatch({
  ...
  const handleChange = useCallback(({ target: { name, value, type } }, shouldSetInitialValue = false) => {
  ...
  return (
    ...
    >
      {isLoadingForData || (!isCreatingEntry && !initialData.id) ? (
        ...
        <>
          <Prompt when={!isEqual(modifiedData, initialData)} message={formatMessage({ id: 'global.prompt.unsaved' })} />
