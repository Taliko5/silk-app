...
import { createDefaultForm, getTrad, getRequestUrl, removePasswordFieldsFromData } from '../../utils';
...
const CollectionTypeFormWrapper = ({ allLayoutData, children, slug, id, origin }) => {
  ...
  const cleanReceivedData = useCallback((data) => {
    ...
    return formatContentTypeData(cleaned, allLayoutDataRef.current.contentType, allLayoutDataRef.current.components);
  }, []);
  ...
  const onDraftRelationCheck = useCallback(async () => {
    try {
      ...
      const endPoint = getRequestUrl(`collection-types/${slug}/${id}/actions/numberOfDraftRelations`);
