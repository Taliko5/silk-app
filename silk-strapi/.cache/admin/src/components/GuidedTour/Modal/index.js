...
const GuidedTourModal = () => {
  const { currentStep, guidedTourState, setCurrentStep, setStepState, isGuidedTourVisible, setSkipped } =
    useGuidedTour();
  const [isVisible, setIsVisible] = useState(currentStep);
  const [{ stepContent, sectionIndex, stepIndex, hasSectionAfter, hasStepAfter }, dispatch] = useReducer(
    reducer,
    initialState
  );
  ...
  if (isVisible && stepContent) {
    return (
      <Modal hideSkip={!hasStepAfter && !hasSectionAfter} onSkip={handleSkip} onClose={handleCtaClick}>
        <StepperModal
          {...stepContent}
