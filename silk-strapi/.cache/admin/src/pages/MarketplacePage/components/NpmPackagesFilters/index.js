  ...
  return (
    <>
      ...
      {query.collections?.map((collection) => (
        <FilterTag name={collection} key={collection} handleRemove={() => handleTagRemove(collection, 'collections')} />
      ))}
      {npmPackageType === 'plugin' &&
        query.categories?.map((category) => (
          <FilterTag name={category} key={category} handleRemove={() => handleTagRemove(category, 'categories')} />
        ))}
    </>
  );
};
