...
function useMarketplaceData({ npmPackageType, debouncedSearch, query, tabQuery }) {
  ...
  const { data: pluginsResponse, status: pluginsStatus } = useFetchMarketplacePlugins(notifyMarketplaceLoad, {
    ...tabQuery.plugin,
    pagination: paginationParams,
    search: debouncedSearch,
  });

  const { data: providersResponse, status: providersStatus } = useFetchMarketplaceProviders(notifyMarketplaceLoad, {
    ...tabQuery.provider,
    pagination: paginationParams,
    search: debouncedSearch,
  });
  ...
  }, [pluginsResponse?.meta.categories, pluginsStatus, npmPackageTypeResponse?.meta.collections, npmPackageTypeStatus]);
