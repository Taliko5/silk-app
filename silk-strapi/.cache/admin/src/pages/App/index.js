...
function App() {
  ...
  const authRoutes = useMemo(() => {
    return makeUniqueRoutes(routes.map(({ to, Component, exact }) => createRoute(Component, to, exact)));
  }, []);
    ...
    <Suspense fallback={<LoadingIndicatorPage />}>
      ...
      <TrackingProvider value={trackingInfo}>
        <Switch>
          ...
          <Route
            path="/auth/:authType"
            render={(routerProps) => <AuthPage {...routerProps} setHasAdmin={setHasAdmin} hasAdmin={hasAdmin} />}
            exact
          />
