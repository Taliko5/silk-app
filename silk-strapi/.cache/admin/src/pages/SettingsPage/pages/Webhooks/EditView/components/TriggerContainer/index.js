  ...
  if (isPending) {
    return (
      <Flex gap={2} alignItems="center">
        <Icon as={Loader} />
        <Typography>{formatMessage({ id: 'Settings.webhooks.trigger.pending', defaultMessage: 'pending' })}</Typography>
      </Flex>
    );
  }
  ...
  if (statusCode >= 200 && statusCode < 300) {
    return (
      <Flex gap={2} alignItems="center">
        ...
        <Typography>{formatMessage({ id: 'Settings.webhooks.trigger.success', defaultMessage: 'success' })}</Typography>
      </Flex>
    );
  }
  ...
  return (
    <Box background="neutral0" padding={5} shadow="filterShadow" hasRadius>
      <Grid gap={4} style={{ alignItems: 'center' }}>
        ...
        <GridItem col={6}>
          {!isPending ? <Message statusCode={statusCode} message={message} /> : <CancelButton onCancel={onCancel} />}
        </GridItem>
      </Grid>
    </Box>
  );
