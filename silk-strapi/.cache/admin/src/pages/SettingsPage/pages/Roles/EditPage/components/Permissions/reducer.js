    ...
    switch (action.type) {
      ...
      case 'ON_CHANGE_COLLECTION_TYPE_GLOBAL_ACTION_CHECKBOX': {
        ...
        Object.keys(get(state, pathToData)).forEach((collectionType) => {
          const collectionTypeActionData = get(state, [...pathToData, collectionType, actionId], undefined);
      ...
      case 'ON_CHANGE_CONDITIONS': {
        Object.entries(action.conditions).forEach((array) => {
          ...
          set(draftState, ['modifiedData', ...stringPathToData.split('..'), 'conditions'], conditionsToUpdate);
        });
