...
const AuthPage = ({ hasAdmin, setHasAdmin }) => {
  ...
  const [{ formErrors, modifiedData, requestError }, dispatch] = useReducer(reducer, initialState, init);
  ...
  const loginRequest = async (body, requestURL, { setSubmitting, setErrors }) => {
    try {
      ...
      if (err.response) {
        const errorMessage = get(err, ['response', 'data', 'error', 'message'], 'Something went wrong');
